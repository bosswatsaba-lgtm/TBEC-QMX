...
const phaseLabels: Record<string, string> = { SUBSTRUCTURE: "Substructure", SUPERSTRUCTURE: "Superstructure", FINISHES: "Finishes" };

interface MeasItem {
  id: string; label: string; phase: string; smm7Code: string | null; value: number; unit: string; active: boolean;
}

export function TakeoffClient({ measurements }: { measurements: MeasItem[] }) {
    ...
    <>
      <header className="bg-surface-container-lowest border-b border-outline-variant flex justify-between items-center w-full h-10 shrink-0 px-4">
          ...
          <div className="ml-4 flex items-center bg-surface-container-low border border-outline-variant">
            {phases.map((p) => (
              <button key={p} onClick={() => setActivePhase(p)}
                className={`px-3 py-1 flex items-center gap-2 text-body-sm font-body-sm transition-colors ${activePhase === p ? "text-primary font-bold bg-surface-container-lowest border-b-2 border-primary-container" : "text-on-surface-variant font-medium hover:bg-surface-container-high"}`}>
        ...
        <div className="flex items-center gap-1">
          <button className="w-8 h-8 flex items-center justify-center text-on-surface-variant hover:bg-surface-container-high transition-colors rounded-none"><span className="material-symbols-outlined text-[20px]">notifications</span></button>
      ...
      <div className="flex-1 flex overflow-hidden">
        <section className="flex-[2] flex flex-col border-r border-outline-variant bg-surface relative">
          <div className="absolute top-4 left-4 z-10 bg-inverse-surface border border-outline-variant p-1 flex flex-col gap-1">
            <button className="p-2 text-secondary-fixed-dim hover:bg-on-secondary-fixed-variant hover:text-primary-fixed transition-colors rounded-none" title="Set Scale"><span className="material-symbols-outlined text-[18px]">straighten</span></button>
            <div className="w-full h-px bg-outline-variant my-0.5"></div>
            <button className="p-2 text-primary-fixed bg-on-secondary-fixed-variant border-l-2 border-primary-container" title="Linear Measurement"><span className="material-symbols-outlined text-[18px]">timeline</span></button>
            <button className="p-2 text-secondary-fixed-dim hover:bg-on-secondary-fixed-variant hover:text-primary-fixed transition-colors rounded-none" title="Area Measurement"><span className="material-symbols-outlined text-[18px]">format_shapes</span></button>
            <button className="p-2 text-secondary-fixed-dim hover:bg-on-secondary-fixed-variant hover:text-primary-fixed transition-colors rounded-none" title="Point Count"><span className="material-symbols-outlined text-[18px]">scatter_plot</span></button>
          </div>
          <div className="flex-1 overflow-auto flex items-center justify-center p-8" style={{ backgroundSize: "40px 40px", backgroundImage: "linear-gradient(to right, #dae2fd 1px, transparent 1px), linear-gradient(to bottom, #dae2fd 1px, transparent 1px)" }}>
            <div className="relative max-w-full max-h-full border border-outline-variant bg-white p-2">
              <div className="w-[480px] h-[360px] bg-surface-container-highest flex items-center justify-center text-on-surface-variant font-body-sm text-body-sm">Blueprint Viewer — Drop DWG/PDF here</div>
              <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 5 }}>
                <path d="M 120,180 L 320,180 L 320,420" fill="none" stroke="#f97316" strokeDasharray="4,4" strokeWidth="2"></path>
                <circle cx="120" cy="180" fill="#f97316" r="4"></circle><circle cx="320" cy="180" fill="#f97316" r="4"></circle><circle cx="320" cy="420" fill="#f97316" r="4"></circle>
          ...
          <div className="h-8 border-t border-outline-variant bg-surface-container-lowest flex items-center justify-between px-3 font-data-mono text-data-mono text-secondary">
            <div className="flex items-center gap-4"><span className="flex items-center gap-1"><span className="w-2 h-2 bg-primary-container"></span> Scale: 1:100</span><span>Snap: ON</span></div>
            <div className="flex items-center gap-2"><span>X: 1450.2</span><span>Y: -820.5</span></div>
          </div>
        </section>
        <section className="flex-1 flex flex-col bg-surface-container-lowest">
            ...
            {items.map((m) => (
              <div key={m.id} className={`border ${m.active ? "border-primary-container bg-surface-container-low" : "border-outline-variant bg-white"} p-2 relative`}>
                ...
                <div className="flex justify-between items-start mb-2 pl-2">
                  <div>
                    <span className="font-data-mono text-data-mono bg-surface-dim text-on-surface px-1 py-0.5">{m.smm7Code ?? m.id.slice(0, 8)}</span>
                    <span className={`font-body-sm text-body-sm ml-2 ${m.active ? "font-semibold" : ""}`}>{m.label}</span>
                  </div>
                  <span className={`font-data-mono text-data-mono ${m.active ? "text-primary-container font-bold" : "text-on-surface"}`}>{m.value} {m.unit}</span>
                </div>
                {m.active ? (
                  <div className="grid grid-cols-2 gap-2 mt-2 pl-2">
                    <div>
                      ...
                      <select className="w-full font-data-mono text-data-mono border-outline-variant py-1 px-2 bg-white rounded-none focus:border-primary-container focus:ring-0">
                        <option>Substructure</option><option>Superstructure</option>
                      </select>
                    </div>
                    <div>
                      <label className="text-[10px] text-secondary uppercase tracking-wider mb-1 block">SMM7 Code</label>
                      <select className="w-full font-data-mono text-data-mono border-outline-variant py-1 px-2 bg-white rounded-none focus:border-primary-container focus:ring-0">
                        <option>E10.1</option><option>F10.111</option><option>F10.2</option>
                      </select>
                    </div>
                  </div>
                ) : (
                  <div className="flex gap-2 mt-1 pl-2">
                    ...
                    {m.smm7Code && <span className="bg-surface-variant px-1 font-body-sm text-body-sm text-secondary">{m.smm7Code}</span>}
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="p-3 border-t border-outline-variant bg-surface-container-low">
            <div className="flex justify-between items-center font-body-sm text-body-sm">
              ...
              <span className="font-bold text-on-surface">{items.filter((m) => m.phase === activePhase).length}</span>
            </div>
