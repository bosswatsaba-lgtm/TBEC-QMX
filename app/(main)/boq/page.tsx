    ...
    <>
      <header className="bg-surface-container-lowest border-b border-outline-variant flex justify-between items-center w-full h-10 shrink-0 px-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <button className="w-8 h-8 flex items-center justify-center text-on-surface-variant hover:bg-surface-container-high transition-colors rounded-none"><span className="material-symbols-outlined text-[20px]">print</span></button>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-1.5 border border-outline-variant bg-surface text-on-surface font-label-caps text-label-caps hover:bg-surface-container-high transition-colors rounded-none flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">filter_list</span>Filter</button>
            <button className="px-3 py-1.5 bg-primary-container text-on-primary font-label-caps text-label-caps hover:bg-primary transition-colors rounded-none flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">download</span>Export</button>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <KpiSmall label="Total Items" value={items.length.toString()} />
          <KpiSmall label="Measured Quantities" value="27,652" />
          <KpiSmall label="Grand Total (USD)" value={`$${grandTotal.toLocaleString()}`} />
          <div className="bg-surface-container-lowest border border-outline-variant p-3 flex flex-col">
            <span className="font-body-sm text-body-sm text-secondary">Export Format</span>
            <select className="mt-1 text-body-lg font-body-lg bg-transparent border border-outline-variant px-2 py-0.5 rounded-none"><option>SMM7</option><option>CESMM4</option><option>NRM2</option></select>
          </div>
        </div>
        <section className="bg-surface-container-lowest border border-outline-variant overflow-hidden flex-1 flex flex-col">
          <div className="bg-surface-container border-b border-outline-variant px-4 py-3 flex items-center gap-2">
            ...
            <span className="ml-auto font-data-mono text-data-mono text-secondary">{items.length} items</span>
          </div>
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {items.map((item, i) => (
                  <tr key={item.id} className={`h-8 border-b border-outline-variant/30 hover:bg-surface-container-highest transition-colors ${i % 2 === 0 ? "bg-surface" : "bg-surface-container-low"}`}>
...
function KpiSmall({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-surface-container-lowest border border-outline-variant p-3 flex flex-col">
      ...
      <span className="font-display text-display text-on-surface mt-1">{value}</span>
    </div>
  );
}
